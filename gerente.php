<?php
    ...
    if(!empty($_GET['pes'])){
	    $data = $_GET['pes'];
	    $s = "select * from pedido_itens where status = 'pendente' and pedido like '%$data%' order by codigo desc";
    }else{
	    $s = "select * from pedido_itens where status = 'pendente' order by codigo desc";
    }
...
<body>
   <header on class="fixo" >
		  ...
		  <tbody> 
			<?php
				while($user_data = mysqli_fetch_assoc($result))
				{
					...
					echo "<td><a class='btn-status' href ='status.php?pedido_id=".$user_data['pedido_id']."'>Finalizar</a></td>";
					echo "</tr>";
				}
			?>
		  </tbody>
	...
	function searchData()
	{
		window.location = 'gerente.php?search='+search.value;
	}
	...
	function pesData()
	{
		window.location = 'gerente.php?pes='+pes.value;
	}
	...
	<!--<section class="section-about" id="area-about">
        ...
        <div class="about">
            <div class="text-about">
                ...
                <p>
                    Página responsável pela administração da cafeteria.
                </p>
            </div>

            <img src="img/logo.png" alt="Logo" class="image-about">
        </div>
